...
router.get('/', async (req, res) => {
    ...
    const radius = req.query.radius || 24;
    ...
    try {
        for (let i = 0; i < terms.length; i++) {
            ...
            const businesses = response.data.businesses.map(business => ({
                name: business.name, rating: business.rating, address: business.location.display_address.join(', '), term: terms[i],//image_url: business.image_url, // wanted to add this but it requires business API
            }));
